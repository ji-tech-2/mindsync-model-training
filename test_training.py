...
def test_dataset():
    ...
    if missing_counts.sum() > 0:
        print(
            f"⚠️  Warning: Missing values found:\n{missing_counts[missing_counts > 0]}"
        )
    else:
...
def test_preprocessing():
    ...
    try:
        ...
        if n_features_selected == 0:
            print(
                "  ⚠️  Warning: Lasso dropped ALL features (data sample might be too small/noisy)"
            )
            ...
            print(
                f"  ✅ Feature selection active (dropped {n_features_expanded - n_features_selected} features)"
            )
